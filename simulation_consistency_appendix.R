# code to conduct simulation 2 and create table for binary outcome paper
...
# Update 2019.04.07 by Tianchen:
# include GEE with exchangeable correlation structure
...
for (i_ss in 1:length(sample_sizes)) {
    ...
    result <- foreach(isim = 1:nsim, .combine = "c") %dorng% {
        ...
        output <- list(list(fit_wcls = fit_wcls, fit_eif = fit_eif, fit_eif_modified = fit_eif_modified,
                            fit_gee_ind = fit_gee_ind, fit_gee_exch = fit_gee_exch))
    }
    sink()
    ...
    names(result_df_beta0) <- names(result_df_beta1) <- c("ss", "est", "bias", "sd", "rmse", "cp.unadj", "cp.adj")
...
result_df_collected <- list(result_df_beta0_collected = result_df_beta0_collected,
                            result_df_beta1_collected = result_df_beta1_collected)

saveRDS(result_df_collected, file = "result_simulation_2.RDS")
...
result_df_collected <- readRDS("result_simulation_2.RDS")
result_df_beta0_collected <- result_df_collected$result_df_beta0_collected
result_df_beta0_collected$beta <- "beta0"
result_df_beta1_collected <- result_df_collected$result_df_beta1_collected
result_df_beta1_collected$beta <- "beta1"

result_df_collected <- rbind(result_df_beta0_collected, result_df_beta1_collected)
...
result_df_collected$bias <- round(result_df_collected$bias, 2)
result_df_collected$sd <- round(result_df_collected$sd, 2)
result_df_collected$rmse <- round(result_df_collected$rmse, 2)
...
table <- data.frame(matrix(NA, nrow = 5 * length(ee_names) * length(sample_sizes), ncol = 5))
names(table) <- c("est", "ss", "beta", "measure", "value")
measures <- c("bias", "sd", "rmse", "cp.unadj", "cp.adj")

i_table <- 1
for (irow in 1:nrow(result_df_collected)) {
    for (measure in measures) {
        table[i_table, "est"] <- result_df_collected$est[irow]
        table[i_table, "ss"] <- result_df_collected$ss[irow]
        table[i_table, "beta"] <- result_df_collected$beta[irow]
        table[i_table, "measure"] <- measure
        table[i_table, "value"] <- result_df_collected[irow, measure]
        i_table <- i_table + 1
    }
}

casted <- cast(table, est + ss ~ beta + measure, value = "value")
casted <- casted[, c(1,2,3,6,7,5,4,8,11,12,10,9)]
casted$est[casted$est == 1] <- "WCLS"
casted$est[casted$est == 2] <- "Eff"
casted$est[casted$est == 3] <- "Eff2"
casted$est[casted$est == 4] <- "GEE.ind"
casted$est[casted$est == 5] <- "GEE.exch"
